<template>
  <div v-if="display" class="comment">
    <div class="media">
      <img :src="item.author.url" alt="ユーザー" class="comment-userimage mr-3" />
      <div class="media-body">
        <div class="comment-header">
          <small class="comment-username text-muted">{{ item.author.name }}</small>
    ...
    <div class="d-flex py-2" style="margin-left: 56px">
      ...
      <span
        v-if="item.is_mine"
        @click="confirmShow"
        class="cursor comment-footer-item align-self-center text-danger"
      >削除</span>
    </div>
